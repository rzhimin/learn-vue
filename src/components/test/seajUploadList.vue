  ...
  <div class="upload-list-style seaj">
    <ul>
      <li class="upload-list-head">
        ...
        <h-row style="flex: 3">
          <h-col class="list-head-item">
            {{ $t("m.i.components.upload.fileName") }}({{
              $t("m.i.components.upload.uploader")
            }}/{{ $t("m.i.components.upload.uploadTime") }})
          </h-col>
        </h-row>
        <h-row>
          <h-col class="list-head-item t-r pd-r-30" style="flex: 1">{{
            $t("m.i.common.action")
          }}</h-col>
        </h-row>
      </li>
      <template v-for="(item, index) in fileDataArray">
        ...
        >
          ...
          <div
            style="width: 25%"
            class="list-body-item pd-l-15"
            :title="item.define_name"
          >
          ...
          <div
            style="width: 160px"
            class="list-body-item pd-l-15 temp-box"
            v-if="item.is_template === '1'"
          >
          ...
          <div class="list-body-item t-r pd-r-20">
            <h-button
              ...
              class="sub"
              v-if="
                openHistory && item.history_file && item.history_file.length > 0
              "
            ...
            <h-button
              ...
              @on-click="
                deleteFile(item.file_id, item.is_all_delete, item.define_type)
              "
              >删除
            </h-button>
            ...
            <span v-if="item.print">
              <h-button
                ...
                @on-click="
                  printFile(item.file_id, item.define_type, item.rule_name)
                "
                :disabled="printIsDis"
                >用印</h-button
              >
            ...
            <span>
              <h-button
                size="small"
                v-if="
                  item.file_name != null && getFIleType(item.file_name) !== '-1'
                "
                class="sub"
                @on-click="
                  prewFile(item.file_id, item.file_name, item.define_type)
                "
                >预览</h-button
              >
            </span>
      ...
      <li
        v-show="otherFileShow"
        class="upload-list-body"
        style="position: relative"
      >
        <div
          class="list-head-item pd-l-15"
          v-if="otherFileName"
      ...
      <li
        class="upload-list-body"
        v-for="(item, index) in otherFileDataArray"
        :key="index"
      >
    ...
    >
      ...
      <seajUploadFileHistory
        :historyFileArray="historyFileArray"
      ></seajUploadFileHistory>
    </h-msg-box>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    acceptFileType: {
      type: String,
      default:
        "bmp,gif,jpg,jpeg,png,pdf,zip,rar,doc,docx,xlsx,xls,7z,txt,ppt,pptx,mp3,mp4,csv",
    },
  },
  computed: {
    fileIds() {
  ...
  methods: {
    downLoadAll() {
      ...
      let other_file_ids =
        this.otherFileIds.length > 0 ? this.otherFileIds.join(",") : null;
    ...
    onExceededize() {
      this.$hMessage.warning(
        `文件大小超出限制！请控制在${parseInt(this.maxSize / 1024)}MB内`
      );
      this.spinShow = false;
    },
    beforeUpload(e, t) {
    ...
    templateDownLoad(item) {
      this.getTemplate(item).then((res) => {
        if (res.error_message === "SUCC") {
          this.downLoad(
            res.result[0].template_fileid,
            item.template_name || res.result[0].template_file_name
          );
        }
      });
    },
    templatePreview(item) {
      this.getTemplate(item).then((res) => {
        if (res.error_message === "SUCC") {
          this.prewFile(
            res.result[0].template_fileid,
            res.result[0].template_file_name
          );
        }
      });
    },
    ...
    reuploadSuccess(fileInfo, file, otherFileDataArray) {
      ...
      if (fileInfo.error_code) {
        this.$hMsgBox.error({
          title: "错误",
          content: fileInfo.error_message
            ? fileInfo.error_message
            : fileInfo.error_code,
        });
        return;
      }
      if (this.workflowConfig.flowKey === "A02615097")
        this.$emit("button-change", true);
      ...
      // 查找当前附件是否存在名称配置
      let config = this.fileDataArray.find(
        (item) => item.define_type === fileInfo.defineType
      );
      ...
      if (config && config.file_name_rule) {
        ...
        if (
          config.file_name_rule.split(".")[1] &&
          fileNameSuffix !== config.file_name_rule.split(".")[1]
        ) {
          this.$hMessage.warning(
            "请上传" + config.file_name_rule.split(".")[1] + "格式文件！"
          );
          return;
        }
        if (config.file_name_rule.split(".")[1]) {
          fileName = config.file_name_rule
            .replace("{fileName}", fileNamePrefix)
            .replace("{currentDate}", currentDate);
        } else {
          fileName =
            config.file_name_rule
              .replace("{fileName}", fileNamePrefix)
              .replace("{currentDate}", currentDate) +
            "." +
            fileNameSuffix;
      ...
      this.relevanceFlie(fileIds, defineType, bizCatalog, fileName).then(
        (res) => {
          // 如果是流程发起节点则不保存历史附件
          ...
          _.each(res, (item) => {
            ...
            if (matchItemIndex === -1) {
              ...
              this.$set(
                this.fileDataArray,
                matchItemIndex,
                Object.assign(this.fileDataArray[matchItemIndex], item)
              );
            }
          });
        }
    ...
    otheruploadSuccess(fileInfo, file, otherFileDataArray) {
      ...
      let workId;
      let bizCatalog =
        this.otherFileType === "fund"
          ? "AMOSPDTFUNDINFO_FILE_OTHER"
          : "AMOSBASEMANGINST_FILE_OTHER";
      let fileIds = [];
      ...
      this.otherFileIds.push(
        fileInfo.fileId ? fileInfo.fileId : fileInfo.file_id
      );
      this.fileDataArray.forEach((item) => {
        workId = item.work_id;
      });
    ...
    validate() {
      ...
      for (let i = 0; i < this.fileDataArray.length; i++) {
        if (
          ...
          (!this.fileDataArray[i].file_name ||
            this.fileDataArray[i].file_name.length === 0)
        ) {
          valid = false;
          this.$hMessage.error(
            "请上传" + this.fileDataArray[i].define_name + "附件！"
          );
          break;
        }
      }
